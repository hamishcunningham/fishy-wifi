...
String apSSIDStr = "AirElf-" + String(ESP.getChipId());
...
String pageTopStr = String(
  "<html><head><title>AirElf Aquaponics Helper [ID: " + apSSIDStr + "]"
);
...
float calcVoltage = 0;
float dustDensity = 0;
boolean GOT_DUST_SENSOR = true; // maybe implement auto detection later
...
void startPeripherals() {
  ...
  //setup dust sensor
    pinMode(16,OUTPUT);  // LED control on pin 16
}
...
void formatMonitorEntry(monitor_t *m, String* buf, bool JSON) {
  ...
  if(GOT_DUST_SENSOR){
    buf->concat("^ ~dust~+ ");
    buf->concat(m->dustDensity);
    if(! JSON) buf->concat("\tug/m3");
  }
  if(JSON) {
    buf->concat(" }");
    buf->replace('~', '"');
...
void getDust(float* dustDensity) {
  digitalWrite(16,LOW); // power on the LED
  delayMicroseconds(280);
  ADCresult = analogRead(A0); // read the dust value
   delayMicroseconds(40);
  digitalWrite(16,HIGH); // turn the LED off
  delayMicroseconds(9680);

  //according to voltage divider, 2.2V should give fsd of 1023
  calcVoltage = ADCresult * (2.2 / 1024.0);
  (*dustDensity) = 170 * calcVoltage - 0.1;
  dbg(monitorDBG, "Dust Density: ");
  dbg(monitorDBG, *dustDensity);
  dln(monitorDBG, " ug/m3");
  return;
}
